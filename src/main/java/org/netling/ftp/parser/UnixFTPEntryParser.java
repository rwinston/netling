    ...
    public FTPFile parseFTPEntry(String entry) {
        ...
        if (matches(entry))
        {
            ...
            case 'd':
                ...
                break;
            case 'e':
                type = FTPFile.Type.SYMBOLIC_LINK;
                break;
            case 'l':
                type = FTPFile.Type.SYMBOLIC_LINK;
                break;
            ...
            for (FTPFile.Access access : FTPFile.Access.values()) {
            	if (!group(g).equals("-"))
                	file.setPermission(access, FTPFile.Permission.READ);
            	if (!group(g + 1).equals("-"))
                	file.setPermission(access, FTPFile.Permission.WRITE);
                final String execPerm = group(g + 2);
                if (!execPerm.equals("-") && !Character.isUpperCase(execPerm.charAt(0)))
                {
                    file.setPermission(access, FTPFile.Permission.EXECUTE);
                }       
                g += 4;	// move the capture group to the next access set
            }
            ...
            else
            {
                ...
                // with spaces in them.
                name += endtoken;
                if (type == FTPFile.Type.SYMBOLIC_LINK)
                {
