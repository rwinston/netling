 ...
 * @version $Id: VMSFTPEntryParser.java 963153 2010-07-11 22:08:56Z sebb $
 *
 * @see org.netling.ftp.FTPFileEntryParser FTPFileEntryParser (for usage instructions)
...
public class VMSFTPEntryParser extends ConfigurableFTPFileEntryParserImpl
{
    ...
    public FTPFile parseFTPEntry(String entry)
    {
        ...
        if (matches(entry))
        {
            ...
            {
                f.setType(FTPFile.Type.DIRECTORY);
            }
            else
            {
                f.setType(FTPFile.Type.FILE);
            }
            //set FTPFile name
            ...
            for (FTPFile.Access access : FTPFile.Access.values()) {
            	final String permission = permissions[idx++];
            	if (permission.indexOf('R')>=0)
            		 f.setPermission(access, FTPFile.Permission.READ);
            	if (permission.indexOf('W')>=0)
            		f.setPermission(access, FTPFile.Permission.WRITE);
            	if (permission.indexOf('E')>=0)
            	     f.setPermission(access, FTPFile.Permission.EXECUTE);
            }
            return f;
        }
        return null;
     ...
     * whatever delimits one entry from the next.   This parser cannot use
