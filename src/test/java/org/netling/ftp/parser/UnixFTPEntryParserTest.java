...
public class UnixFTPEntryParserTest extends FTPParseTestFramework {
    ...
    /**
     * @see org.netling.ftp.parser.FTPParseTestFramework#getBadListing()
     */
    @Override
    ...
    /**
     * @see org.netling.ftp.parser.FTPParseTestFramework#getGoodListing()
     */
    @Override
    ...
    /**
     * @see org.netling.ftp.parser.FTPParseTestFramework#getParser()
     */
    @Override
    ...
    public void testNoSpacesBeforeFileSize() {
        ...
        assertEquals(f.getType(), FTPFile.Type.DIRECTORY);
    ...
    /**
     * @see org.netling.ftp.parser.FTPParseTestFramework#testParseFieldsOnDirectory()
     */
    @Override
    ...
    protected void doAdditionalGoodTests(String test, FTPFile f) {
        ...
        if (null != link) {
            ...
            if (linklen > 0) {
                ...
                assertEquals(f.getType(), FTPFile.Type.SYMBOLIC_LINK);
            }
        }
        FTPFile.Type type = f.getType();
        switch (test.charAt(0))
        {
        case 'd':
            assertEquals("Type of "+ test, type, FTPFile.Type.DIRECTORY);
            break;
        case 'l':
             assertEquals("Type of "+ test, type, FTPFile.Type.SYMBOLIC_LINK);
             break;
        case 'b':
        case 'c':
        ...
        case 'f':
        case '-':
            assertEquals("Type of "+ test, type, FTPFile.Type.FILE);
            break;
        default:
            assertEquals("Type of "+ test, type, FTPFile.Type.UNKNOWN);
        }

        int acc = 0; 
        for (FTPFile.Access access : FTPFile.Access.values())
        {
        	int p = 0;
            for (FTPFile.Permission perm : FTPFile.Permission.values())
            {
                int pos = 3*acc + (p++) + 1;
