...
public class VMSFTPEntryParserTest extends FTPParseTestFramework
{
     ...
     * @param dir
     */
    private void checkPermissions(FTPFile dir, int octalPerm)
    {
        int permMask = 1<<8;
        assertTrue("Owner should not have read permission.",
                ((permMask & octalPerm) != 0) == dir.hasPermission(FTPFile.Access.USER,
                                      FTPFile.Permission.READ));
        permMask >>= 1;
        assertTrue("Owner should not have write permission.",
                ((permMask & octalPerm) != 0) == dir.hasPermission(FTPFile.Access.USER,
                                      FTPFile.Permission.WRITE));
        permMask >>= 1;
        assertTrue("Owner should not have execute permission.",
                ((permMask & octalPerm) != 0) == dir.hasPermission(FTPFile.Access.USER,
                                      FTPFile.Permission.EXECUTE));
        permMask >>= 1;
        assertTrue("Group should not have read permission.",
                ((permMask & octalPerm) != 0) == dir.hasPermission(FTPFile.Access.GROUP,
                                      FTPFile.Permission.READ));
        permMask >>= 1;
        ...
        assertTrue("World should not have read permission.",
                ((permMask & octalPerm) != 0) == dir.hasPermission(FTPFile.Access.WORLD,
                                      FTPFile.Permission.READ));
        permMask >>= 1;
